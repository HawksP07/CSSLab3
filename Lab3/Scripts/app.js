...
((core) =>
{
    ...
    function DisplayTaskList() {
      ...
      $("ul").on("click", ".editButton", function () {
          ...
          let text = $(this).parent().parent().children("#taskText");
          editText.val(text.text()).show().trigger("select");
      });
      ...
      $("ul").on("keypress", ".editTextInput", function (event) {
          if (event.key == "Enter") {
              let editText = $(this);
              let taskText = editText.parent().children("#taskText");
              if (editText.val() != "" && editText.val().charAt(0) != " ") {
                  taskText.text(editText.val());
                  editText.hide();
                  messageArea.removeAttr("class").hide();
              } else {
